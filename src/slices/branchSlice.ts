  ...
  async (branchData, { rejectWithValue, dispatch }) => {
    try {
      ...
      const response = await axios.post(
        `${SERVER_DOMAIN}/branch/createBranch`,
        branchData,
        {
          headers: {
...
>(
  ...
  async ({ branchId, reason }, { rejectWithValue, dispatch }) => {
    try {
      const token = localStorage.getItem("token");
      await axios.delete(
...
export const fetchBranchById = createAsyncThunk<
  ...
  string,
  { rejectValue: string }
>("branches/fetchBranchById", async (branchId, { rejectWithValue }) => {
  try {
    const token = localStorage.getItem("token");
    const response = await axios.get(
      `${SERVER_DOMAIN}/branch/getBranch/${branchId}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  async (branchData, { rejectWithValue, dispatch }) => {
    try {
      ...
      const response = await axios.put(
        `${SERVER_DOMAIN}/branch/editBranch`,
        branchData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        ...
        (state, action: PayloadAction<{ branchId: string }>) => {
          state.loading = false;
          state.branches = state.branches.filter(
            (branch) => branch._id !== action.payload.branchId
          );
        }
      )
      .addCase(
...
export const { resetBranchNotFound, userSelectedBranch, clearSelectedBranch } =
  branchSlice.actions;
